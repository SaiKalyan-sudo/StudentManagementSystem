...
public class Sign_up extends Frame implements ActionListener
{
    ...
    Sign_up()
    {
        ...
        l7.setBounds(50, 280, 100, 50);
        f.add(l7); 
        t7=new TextField("");
        t7.setBounds(200, 290, 100, 30);
        f.add(t7);
        ...
        try
        {
            if(e.getSource()==b1)
            {
                ...
                int m=Integer.parseInt(number);
                    s1="insert into studetails(Regno,Stuname,Password,Gmail,Phone_no,Course,Address) values('"+id+"','"+name+"','"+password+"','"+mail+"','"+number+"','"+course+"','"+address+"')";
                    s=c.createStatement();
                    s.executeUpdate(s1);
